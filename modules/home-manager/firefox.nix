{pkgs, ...}:
let
  # Chrome + prefs shared by every profile below. Profile *state*
  # (cookies, logins, history, containers) stays separate — that is the
  # whole point of having more than one profile.
  commonProfile = {
...
in
{
  programs.firefox = {
    ...
    profiles.dave = commonProfile // {
      id = 0;
      isDefault = true;
    };

    # Second identity: separate cookie jar / logins, same chrome.
    # Launch with: firefox -P anthropic-2
    profiles.anthropic-2 = commonProfile // {
      id = 1;
      isDefault = false;
    };
  };
