...
}:
{
  ...
  exports = lib.mapAttrs' (instanceName: _: {
    ...
    value = {
      ...
      networking.module = "clan_lib.network.p2p_ssh";
    };
  }) config.instances;

  roles.server = {
    description = "Expose sshd via dumbpipe over iroh";

    perInstance =
      { instanceName, mkExports, machine, ... }:
      {
        exports = mkExports {
          peer.hosts = [
            {
              plain = clanLib.getPublicValue {
                ...
                generator = "p2p-ssh-${instanceName}";
                file = "ticket";
                flake = directory;
              };
            }
        ...
        nixosModule =
          { config, pkgs, ... }:
          let
            generatorName = "p2p-ssh-${instanceName}";
          ...
          in
          {
            ...
            systemd.services."p2p-ssh-${instanceName}" = {
              ...
              serviceConfig = {
                ...
                ExecStart = let
                  wrapper = pkgs.writeShellScript "p2p-ssh-${instanceName}" ''
                    export IROH_SECRET
