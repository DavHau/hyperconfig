# Shared building blocks for the omp wrappers (pi.nix, pi-superpowers.nix):
# the locally-patched omp package, the base config.yml text (parametrized on
# skill directories), the jj AGENTS.md, and the optional llama-swap models.yml.
# Imported as a plain function, NOT a NixOS module.
...
in rec {
  ...
  # Base config.yml shared by both wrappers; only the skill directories
  # differ (mattpocock categories for omp, the superpowers library for
  # omp-sp). See pi.nix for the per-setting rationale comments preserved
  # inline below.
  mkConfigFile = skillDirs: pkgs.writeText "config.yml" ''
    startup:
      quiet: true
  ...
  # Required by the jobs-hub extension; upstreamable.
